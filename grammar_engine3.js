		...
		if(production.symbols[0].symbol === startingSymbol) {			
			if(production.symbols.length > 1) {
				...
				if(production.symbols[1].nonTerminal === false) {
					terminals.push(new UserSelection(originalGrammarName, production.symbols[1].symbol, 1, index, true));
				}
				else  {
			...
			_getSuggestionsFromProductionsStartingWithThisSymbol(production.symbols[0].symbol, startingSymbol, processedGrammars, terminals);
		}
	}
}
